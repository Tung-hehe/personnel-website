...
export function Pre({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div ref={textInput} onMouseEnter={onEnter} onMouseLeave={onExit} className="relative">
      {hovered && (
        <>
          <button
            ...
            type="button"
            className={clsx(
              `absolute z-10 right-2 top-2 w-8 h-8 p-1 rounded border bg-gray-800`,
          ...
          {copied && (
            <>
              <span className="copy-text absolute right-12 top-2.5 z-10 rounded-md bg-gray-300 px-1.5 py-0.5 text-gray-800">
                Copied
              </span>
      ...
      <pre className="max-h-[40rem] overflow-y-auto">{children}</pre>
    </div>
  )
}
