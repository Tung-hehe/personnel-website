...
export function SkyscraperLevelSelect({ puzzles, completed, locale, onSelect }: SkyscraperLevelSelectProps) {
  ...
  return (
    <div>
      <p className="mb-4 text-gray-400">{skyscraperConfig.rules[locale]}</p>
      <div className="mb-8 rounded-xl border border-gray-700 bg-primary-dark/30 p-4">
        <div className="mb-2 flex items-center justify-between text-sm">
          <span className="text-gray-300">{skyscraperConfig.chooseLevel[locale]}</span>
      ...
      {skyscraperDifficulties.map((difficulty) => {
        ...
        return (
          <div key={difficulty} className="mb-8">
            <h2 className="mb-3 flex items-center gap-2 text-lg font-bold text-gray-100">
              ...
              {skyscraperConfig.difficulty[difficulty][locale]}
              ...
              {levels.map((puzzle, i) => {
                ...
                return (
                  <button
                    ...
                    title={isDone ? skyscraperConfig.completed[locale] : `${puzzle.size}x${puzzle.size}, ${clueCount} ${skyscraperConfig.clues[locale]}`}
                    aria-label={`${skyscraperConfig.level[locale]} ${i + 1}`}
                    className={clsx(
