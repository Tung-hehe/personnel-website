...
- $N(i, j)$ là tập hợp các ô cạnh ô $(i, j)$ (theo cả hướng chéo)
- $f(i, j)$ là số ô kề ô $(i, j)$ (theo cả hướng chéo)
...
- Một ngôi sao không nằm cạnh bất kỳ ngôi sao nào khác
  ...
  ```math
    x(i, j) = 1 \Rightarrow \sum\limits_{\forall (p, q) \in N_(i, j)}{x(p, q)} = 0, \forall 0 \leq i < n_R, 0 \leq j < n_C
  ```
  ...
  ```math
    f_(i, j) \cdot x(i, j) + \sum\limits_{\forall (p, q) \in N_(i, j)}{x(p, q)} \leq f_(i, j), \forall 0 \leq i < n_R, 0 \leq j < n_C
  ```
