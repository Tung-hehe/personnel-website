...
Với mỗi ô $(i, j)$ trên lưới và một thiên hà $g$ ($0 \leq g < n_G$), xây dựng biến $x(i, j, g)$ thỏa mãn
```math
x(i, j, g) = \begin{cases}
...
```math
\sum_{g = 0}^{n_G - 1}{x(i, j, g)} = 1,  \\ \forall 0 \leq i < n_R, 0 \leq j < n_C
```
...
thiên hà đó.
  ...
  Để tuyến tính hóa điều kiện này ta phải tạo thêm các biến trung gian $$t_{ijgp}$$ với mỗi thiên hà
