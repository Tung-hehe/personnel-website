...
- Với mỗi đường nằm ngang $(i, j)$ trong lưới ($0 \leq i < n_R + 1, 0 \leq j < n_C$), xây dựng biến $h(i, j)$ thỏa mãn
```math
h(i, j) = \begin{cases}
...
- Với mỗi đường nằm dọc $(i, j)$ trong lưới ($0 \leq i < n_R, 0 \leq j < n_C + 1$), xây dựng biến $v(i, j)$ thỏa mãn
```math
v(i, j) = \begin{cases}
  1 & \text{nếu đường } (i, j) \text{ được kết nối} \\
  0 & \text{ngược lại}
...
- Các điểm phải nối với nhau thành một chu trình khép kín, không chồng chéo, rẽ nhánh.
  ...
  Để mô hình hóa điều kiện này một cách trực tiếp là một điều không hề dễ dàng, vì vậy
...
Khi giải mô hình trên, ta thu được một lời giải đã thỏa mãn hầu hết các điều kiện của bài toán
